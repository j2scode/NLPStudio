...
#'   \item{\code{getChildren()}}{Not implemented for this class.}
...
Document <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(name, fileName, desc = NULL) {
      ...
      if (missing(fileName)) {
        v <- Validate0$new()
        v$notify(class = "Document", method = "initialize", fieldName = "fileName",
                 value = "", level = "Error",
                 msg = paste("File anme parameter is missing with no default.",
                             "See ?Document for further assistance."),
                 expect = NULL)
        stop()
      }
